 ...
 *
 * @param {string | URL} url `string` or `URL` instance
 * @throws {Error} if `url` is not a `string` or an `URL` object
 *
 * ### Example
 * ```ts
...
function useUrl(url) {
  ...
  return [newUrl, newUrl.searchParams]
}
...
// attach params to an URL
const url = new URL('http://example.com')
url.search = urlObjectFromObject.toString()
