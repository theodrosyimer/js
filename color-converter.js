...
// CHORE: finish to refactor the code
;(function () {
  ...
  w3color.prototype = {
    ...
    toHslString() {
      return `hsl(${this.hue}, ${Math.round(this.sat * 100)}%, ${Math.round(
        this.lightness * 100
      )}%)`
    },
    toHslStringDecimal() {
    ...
    toCmykString() {
      return `cmyk(${Math.round(this.cyan * 100)}%, ${Math.round(
        this.magenta * 100
      )}%, ${Math.round(this.yellow * 100)}%, ${Math.round(this.black * 100)}%)`
    },
    toCmykStringDecimal() {
  ...
  function toColorObject(c) {
    ...
    if (c.length !== 3 && c.length !== 6 && !Number.isNaN(c)) {
      c = `ncol(${c})`
    }
...
// source: [Converting Color Spaces in JavaScript | CSS-Tricks - CSS-Tricks](https://css-tricks.com/converting-color-spaces-in-javascript/)
